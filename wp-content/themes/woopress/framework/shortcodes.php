    ...
    ?>
        ...
        <form action="<?php the_permalink(); ?>" method="get" id="contact-form" class="contact-form <?php echo $class; ?>">
            ...
            <div class="form-group">
              <p class="form-name">
                <label for="name" class="control-label"><?php _e('Nombre y Apellido', ETHEME_DOMAIN) ?> <span class="required">*</span></label>
            ...
            <div class="form-group">
              <p class="form-name">
                <label for="contact-website" class="control-label"><?php _e('Teléfono / Celular', ETHEME_DOMAIN) ?></label>
            ...
            <div class="form-group">
              <p class="form-textarea">
                <label for="contact_msg" class="control-label"><?php _e('Mensaje', ETHEME_DOMAIN); ?> <span class="required">*</span></label>
            ...
            <p class="pull-right">
              ...
              <span class="spinner"><?php _e('Enviando...', ETHEME_DOMAIN) ?></span>
              <button class="btn btn-black big" id="submit" type="submit"><?php _e('Enviar Mensaje', ETHEME_DOMAIN) ?></button>
            </p>
	...
	function etheme_search($atts) {
		...
		$output .= '<div class="et-mega-search '.$class.'" data-products="'.$products.'" data-count="'.$count.'" data-posts="'.$posts.'" data-portfolio="'.$portfolio.'" data-pages="'.$pages.'" data-images="'.$images.'">';
			$output .= '<form method="get" action="'.home_url( '/' ).'">';
				$output .= '<input type="text" value="'.$search_input.'" name="s" id="s" autocomplete="off" placeholder="'.__('Buscar', ETHEME_DOMAIN).'"/>';
				$output .= '<input type="hidden" name="post_type" value="'.$post_type.'"/>';
				$output .= '<input type="submit" value="'.__( 'Ir', ETHEME_DOMAIN ).'" class="button active filled"  /> ';
			$output .= '</form>';
...
if(!function_exists('et_twitter_slider')) {
	function et_twitter_slider($atts) {
		...
		if(empty($consumer_key) || empty($consumer_secret) || empty($user_token) || empty($user_secret) || empty($user)) {
			return __('Sin información', ETHEME_DOMAIN);
		}
...
function etheme_new_shortcodes($atts, $content=null){
    ...
    extract(shortcode_atts(array( 
        ...
        'limit' => 50,
        'categories' => '',
        'title' => __('Últimos Productos', ETHEME_DOMAIN)
    ), $atts)); 
        ...
        <div class="project-navigation">
            <?php if(!empty($prev_post)) : ?>
                <div class="pull-left prev-project">
                    <a href="<?php echo get_permalink($prev_post->ID); ?>" class="btn border-grey btn-xmedium project-nav"><?php _e('Anterior', ETHEME_DOMAIN); ?></a> 
                    <div class="hide-info">
            ...
            <?php if(!empty($next_post)) : ?>
                <div class="pull-right next-project">
                    <a href="<?php echo get_permalink($next_post->ID); ?>" class="btn border-grey btn-xmedium project-nav"><?php _e('Siguiente', ETHEME_DOMAIN); ?></a>
                    <div class="hide-info">
...
function etheme_share_shortcode($atts, $content = null) {
	...
	$html .= '
		<ul class="menu-social-icons '.$class.'">
			...
			<li>
				<a href="mailto:enteryour@addresshere.com?subject='.$post_title.'&amp;body=Check%20this%20out:%20'.$permalink.'" class="'.$tooltip_class.'" title="'.__('Mail a un amigo', ETHEME_DOMAIN).'" target="_blank">
...
function etheme_follow_shortcode($atts, $content = null) {
	...
	extract(shortcode_atts(array(
		"title" => '',
		'twitter' => '',
		'facebook' => 'https://www.facebook.com/portezuelo.oficial',
		'pinterest' => '',
		'email' => '',
	), $atts));
	ob_start();
    ...
    <div class="social-icons">
    	...
    	<?php if($facebook){?> 
    	<a href="<?php echo $facebook; ?>" target="_blank"  class="icon facebook tip-top" data-tip="Seguinos en Facebook"><span class="icon-facebook"></span></a>
		<?php }?>
		<?php if($twitter){?> 
		       <a href="<?php echo $twitter; ?>" target="_blank" class="icon twitter tip-top" data-tip="Seguinos en Twitter"><span class="icon-twitter"></span></a>
		<?php }?>
		<?php if($email){?> 
		       <a href="mailto:<?php echo $email; ?>" target="_blank" class="icon email tip-top" data-tip="Envíanos un email"><span class="icon-envelop"></span></a>
		<?php }?>
		<?php if($pinterest){?> 
		       <a href="<?php echo $pinterest; ?>" target="_blank" class="icon pintrest tip-top" data-tip="Seguinos en Pinterest"><span class="icon-pinterest"></span></a>
		<?php }?>
     </div>
...
function etheme_team_member_shortcode($atts, $content = null) {
    $a = shortcode_atts(array(
        ...
        'email' => '',
        'twitter' => '',
        'facebook' => 'https://www.facebook.com/brunofedericofernandez',
        'skype' => '',
        'linkedin' => '',
        'instagram' => '',
        'position' => '',
...
function etheme_featured_post_shortcode($atts) {
        ...
        <?php while ( $the_query->have_posts() ) : $the_query->the_post();  $postId = get_the_ID(); ?>
            ...
            <div class="featured-posts <?php echo $a['class']; ?>">
                <?php if ($a['title'] != ''): ?>
                    ...
                    <?php if ($a['more_posts']): ?>
                            <?php echo '<a href="'.$posts_url.'" class="show-all-posts hidden-tablet hidden-phone">'.__('Ver más entradas', ETHEME_DOMAIN).'</a>'; ?>
                    <?php endif ?>
                <?php endif ?>
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="post-info">
                            <span class="posted-on">
                                <?php _e('Posteado el', ETHEME_DOMAIN) ?>
                                <?php the_time(get_option('date_format')); ?> 
                                <?php _e('a las', ETHEME_DOMAIN) ?> 
                                <?php the_time(get_option('time_format')); ?>
                            </span> 
                            <span class="posted-by"> <?php _e('por', ETHEME_DOMAIN);?> <?php the_author_posts_link(); ?></span>
                        </div>
                        <div class="post-description">
                            <?php the_excerpt(); ?>
                            <a href="<?php the_permalink(); ?>" class="button read-more"><?php _e('Leer más', ETHEME_DOMAIN) ?></a>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <?php else:  ?>
        ...
        <p><?php _e( 'Lo sentimos, no hay entradas que coincidan con tu criterio de búsqueda.', ETHEME_DOMAIN ); ?></p>
