...
if(!function_exists('et_email_btn')) {
    function et_email_btn($label = '') {
        ...
        if($label == '') {
            $label = __('Email a un amigo', ETHEME_DOMAIN);
        }
        $html .= '
    ...
    function et_size_guide() {
	    if ( etheme_get_custom_field('size_guide_img') ) : ?>
	        ...
	        <div class="size_guide">
	    	 <a rel="<?php echo $lightbox_rel; ?>" href="<?php etheme_custom_field('size_guide_img'); ?>"><?php _e('¿Qué talle soy?', ETHEME_DOMAIN); ?></a>
	        </div>
	    <?php endif;	
    }
}
	...
	function etheme_wc_get_product_labels( $product_id = '' ) {
	    ...
	    if ( etheme_get_option('new_icon') ) : $count_labels++; 
	        if(etheme_product_is_new($product_id)) :
	            ...
	            $output .= '<span class="label-icon new-label '.$second_label.'">'.__( 'Nuevo!', ETHEME_DOMAIN ).'</span>';
	        endif;
	    endif; 
	    return $output;
	}
			...
			<div class="shopping-container" <?php if(etheme_get_option('favicon_badge')) echo 'data-fav-badge="enable"' ?>>
				<div class="shopping-cart-widget" id='basket'>
					...
					<div class="cart-bag">
						...
						<span class='shop-text'><?php _e('Carro', ETHEME_DOMAIN) ?>:</span> <span class="total"><?php echo $woocommerce->cart->get_cart_subtotal(); ?></span>
					</a>
				</div>
...
if(!function_exists('etheme_cart_items')) {
	function etheme_cart_items ($limit = 3) {
        ...
        if ( sizeof( $woocommerce->cart->get_cart() ) > 0 ) {
          ?>
			<p><?php _e('Productos añadidos recientemente', ETHEME_DOMAIN); ?></p>
			<ul class='order-list'>
          <?php
            $counter = 0;
            foreach ( $woocommerce->cart->get_cart() as $cart_item_key => $cart_item ) {
                ...
                ?>
					<li>
						<?php 
                            echo apply_filters( 'woocommerce_cart_item_remove_link', sprintf('<a href="%s" data-key="%s" class="close-order-li" title="%s"></a>', esc_url( $woocommerce->cart->get_remove_url( $cart_item_key ) ), $cart_item_key, __('Eliminar este producto', ETHEME_DOMAIN) ), $cart_item_key ); 
                        ?>
						<div class="media">
        ...
        <?php   
            ...
            echo '<p class="empty a-center">' . __('No tiene productos en el carro.', ETHEME_DOMAIN) . '</p>';
        }
        ...
        if ( sizeof( $woocommerce->cart->get_cart() ) > 0 ) {
          ...
          ?>
			<p class="small-h pull-left"><?php echo __('Subtotal', ETHEME_DOMAIN); ?></p>
			<span class="big-coast pull-right">
				<?php echo $woocommerce->cart->get_cart_subtotal(); ?>
			</span>
			<div class="clearfix"></div>
			<div class='bottom-btn'>
				<a href="<?php echo $woocommerce->cart->get_cart_url(); ?>" class='btn text-center border-grey'><?php echo __('Ver Carro', ETHEME_DOMAIN); ?></a>
				<a href="<?php echo $woocommerce->cart->get_checkout_url(); ?>" class='btn text-center big filled'><?php echo __('Comprar', ETHEME_DOMAIN); ?></a>
			</div>
